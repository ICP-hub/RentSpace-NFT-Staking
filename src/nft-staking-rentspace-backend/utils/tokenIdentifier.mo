...
module TokenIdentifier = {
    ...
    public func isPrincipal(tid : EXT.TokenIdentifier, p : Principal) : Bool {
      let tobj = decode(tid);
      return Blob.equal(Blob.fromArray(tobj.canister), Principal.toBlob(p));
    };
    public func getIndex(tid : EXT.TokenIdentifier) : EXT.TokenIndex {
      let tobj = decode(tid);
      tobj.index;
    };
    public func decode(tid : EXT.TokenIdentifier) : TokenObj {

export const idlFactory = ({ IDL }) => {
  ...
  const Metadata = IDL.Variant({
    ...
    'nonfungible' : IDL.Record({ 'metadata' : IDL.Text }),
  });
  ...
  const Result_3 = IDL.Variant({ 'ok' : TokenIndex, 'err' : CommonError });
  const Result_2 = IDL.Variant({ 'ok' : Metadata, 'err' : CommonError });
  const MintRequest = IDL.Record({ 'to' : User, 'metadata' : IDL.Text });
  ...
  const erc721_token = IDL.Service({
    ...
    'index' : IDL.Func([TokenIdentifier__1], [Result_3], ['query']),
